...
Install SSH, SSH credentials, and SSH build agent plugins in Jenkins
...
From Triggers > Build periodically > Add cronjob H/6 * * * *
...
Add Build Steps > Execute shell script on remote host using ssh > add below lines:
     ...
     echo "app 1 pass" | sudo -S sshpass -p "storage-server-pass" scp -o StrictHostKeyChecking=no -r /var/log/httpd/* natasha@ststor01:/usr/src/security
